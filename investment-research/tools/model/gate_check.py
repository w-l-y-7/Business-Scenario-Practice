...
用法：
    ...
    python tools/model/gate_check.py \
...
def parse_status_block(path):
    """读出 Markdown 文件里第一个 YAML 状态块，读不到返回 None。"""
    try:
    ...
    for path, block in deprecated:
        ...
        for value in [v.strip() for v in values.strip("[]").split(",") if v.strip()]:
            if value in report_text:
                problems.append(
                    f"废弃内容：成品里出现了被作废的数字 `{value}`（来自 {path}）。"
                )
...
def main(argv=None):
    ...
    problems += check_deprecated(collect_deprecated(args.scan), report_text)
